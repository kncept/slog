...
import { extractDomainNameFromUrl, fullyQualifiedApiDomainName, fullyQualifiedFrontendDomainName, listDomainNames } from '../tools/domain-tools'
...
async function defineStacks() {
  ...
  const hostedZoneStack = new HostedZoneStack(app, `${stackNameForHostedZone()}`, {
    hostedZoneName: extractDomainNameFromUrl(),
    hostedZoneIdLookup: await determineHostedZoneIdLookup()
  })
